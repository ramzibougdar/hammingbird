...
<div class="megasocialconnect">
    <div class="text-center">
        <label class="form-label mt-2 mb-0">{l s='or sign in with' mod='pssocialconnect'}</label>
    </div>
        ...
        <div class="col">
            <div class="pssocialconnect-row">
                {if $socialconnect_params.PSSOCIALCONNECT_FACEBOOK_SWITCH eq 1 && !empty($socialconnect_params.PSSOCIALCONNECT_FACEBOOK_API_KEY)}
                    {* facebook button *}
                    {include file="module:pssocialconnect/views/templates/hook/ps_socialconnect_facebook_button.tpl" short_title = 1}
                {/if}
            </div>
        </div>
        <div class="col">
            <div class="pssocialconnect-row">
                {if $socialconnect_params.PSSOCIALCONNECT_GOOGLE_SWITCH eq 1 && !empty($socialconnect_params.PSSOCIALCONNECT_GOOGLE_API_KEY)}
                    {* google button *}
                    {include file="module:pssocialconnect/views/templates/hook/ps_socialconnect_google_button.tpl" short_title = 1}
                {/if}
            </div>
        </div>
    ...
    <div class="pssocialconnect-row minirow">
        {if $socialconnect_params.PSSOCIALCONNECT_AMAZON_SWITCH eq 1 && !empty($socialconnect_params.PSSOCIALCONNECT_AMAZON_API_KEY)}
            ...
            {* amazon button *}
                {include file="module:pssocialconnect/views/templates/hook/ps_socialconnect_amazon_button.tpl" short_title = 1}
        </span>
        {/if}
        ...
        {if $socialconnect_params.PSSOCIALCONNECT_PAYPAL_SWITCH eq 1 && !empty($socialconnect_params.PSSOCIALCONNECT_PAYPAL_API_KEY)}
            ...
            {* paypal button *}
                {include file="module:pssocialconnect/views/templates/hook/ps_socialconnect_paypal_button.tpl" short_title = 1}
        </span>
        {/if}
        ...
        {if $socialconnect_params.PSSOCIALCONNECT_LINKEDIN_SWITCH eq 1 && !empty($socialconnect_params.PSSOCIALCONNECT_LINKEDIN_API_KEY)}
            ...
            {* linkedin button *}
                {include file="module:pssocialconnect/views/templates/hook/ps_socialconnect_linkedin_button.tpl" short_title = 1}
        </span>
        {/if}
